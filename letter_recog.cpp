...
#define IMG_SAMPLE_SCALE (0.7)
//#define IMG_READ_CHANNELS (IMREAD_GRAYSCALE)
#define IMG_READ_CHANNELS (IMREAD_COLOR)
...
static void batch_resize_img(const string& imgfolder)
{
	...
	//{
	//	src = cv::imread(filenames[i].c_str(), IMREAD_COLOR);
	//	cv::resize(src, dst, Size(100, 100));
	//}
}

static void elbp(Mat& src, Mat &dst, int radius, int neighbors)
{
...
static void cvtToHSV(const cv::Mat& src, cv::Mat &Dst)
{
	cv::Mat temp;
	cv::normalize(src, temp, 0, 255, cv::NORM_MINMAX, CV_8UC3); //归一化处理
	//imshow("pic1", src);
	//imshow("pic2", temp);
	//waitKey(0);
	cv::Mat img;
	cv::resize(temp, img, Size(IMG_COLS, IMG_ROWS), INTER_LINEAR);

	cv::cvtColor(img, Dst, COLOR_BGR2HSV);
	return;
}

static void cvtToHSV_LBP(const cv::Mat& src, cv::Mat &Dst)
{
	cv::Mat img;
	cv::Mat img_hsv;
	cv::Mat img_lbp;
	std::vector<cv::Mat> rgbChannels(IMG_CHANNELS);
	std::vector<cv::Mat> rgbLbpChannels;

	cv::resize(src, img, Size(IMG_COLS, IMG_ROWS), INTER_LINEAR);
	...
	{
		Mat img_lbp = Mat(img.rows, img.cols, CV_8UC1, Scalar(0));
		elbp(rgbChannels[i], img_lbp, 1, 8);
		rgbLbpChannels.push_back(img_lbp);
	}

	cv::merge(rgbLbpChannels, Dst);

	return;
}

static void read_imgList(const string& filename, vector<Mat>& images, vector<int>& labels) {
	...
	while (getline(file, line))
	{
		...
		src = cv::imread(result[0], IMG_READ_CHANNELS);

		cv::Mat dst;
		//cv::resize(src, dst, Size(IMG_COLS, IMG_ROWS), INTER_LINEAR);
		cvtToHSV(src, dst);
		//cvtToHSV_LBP(src, dst);
...
static  Mat formatImagesForSVM(const vector<Mat> &data)
{
	Mat dst(static_cast<int>(data.size()), data[0].rows*data[0].cols*IMG_CHANNELS, CV_32F);
...
static Mat formatFeatureByPca(const Mat &dataRaw, const PCA &pca)
{
	Mat point = pca.project(dataRaw.row(0));
	int nsample = dataRaw.rows;
	int FeatureDim = point.cols;
	Mat data(nsample, FeatureDim, CV_32F);
	for (int i = 0; i < nsample; i++)
	{
		Mat pca_point = pca.project(dataRaw.row(i));
		//cout << pca_point << endl;
		Mat row_i = data.row(i);
		pca_point.convertTo(row_i, CV_32F);
	}
	return data;
}
                      ...
                      const string& filename_to_load )
{
 ...
 //   Mat responses;
	...
	//bool ok = read_num_class_data("letter-recognition.data", 16, &data, &responses);
	//if (!ok)
	//	return ok;
	...
	int iSamples = 0;
	string pcaFileSave = "";
	string pcaFileLoad = "pca.xml";

	read_imgList("F:/nazhi_Daniel/C++/opencv_svm/opencv_svm/out_2000.txt", images, labels);

	iSamples = images.size();
	Mat responses = Mat(iSamples, 1, CV_32S);
	memcpy(responses.data, labels.data(), labels.size()*sizeof(int));
	Mat dataRaw = formatImagesForSVM(images);

	//Mat dataTrainRoI = dataRaw.rowRange(0, 50);
	// perform PCA

	//if (pcaFileSave != "")
	//{
	//	PCA pca(dataRaw, cv::Mat(), PCA::DATA_AS_ROW, 0.95);
	//	savePCAdata(pca, pcaFileSave);
	//	return true;
	//}
	//PCA pca;
	//if (pcaFileLoad != "")
	//{
	//	loadPCAdata(pca, pcaFileLoad);
	//}

	//PCA dataRaw to data
	//Mat data = formatFeatureByPca(dataRaw, pca);
	//Mat data = pca.project(dataRaw);

	Mat data = dataRaw;
    Ptr<SVM> model;

    int nsamples_all = data.rows;
	int ntrain_samples = (int)(nsamples_all*IMG_SAMPLE_SCALE);
...
static Ptr<SVM> liveness_load_SVM_model()
{
	Ptr<SVM> model = load_classifier<SVM>("svm_para");
	return model;
}

static PCA liveness_load_PCA_model()
{
	PCA pca;
	loadPCAdata(pca, "pca.xml");
	return pca;
}

static float liveness_test(string imgPath, const PCA &pca, const Ptr<SVM> model)
{
	Mat img = imread(imgPath, IMG_READ_CHANNELS);
	...
	Mat img1;
	cvtToHSV(img, img1);
	Mat image_row = img1.clone().reshape(1, 1);
	Mat pca_point = pca.project(image_row);

	float r = model->predict(pca_point);
